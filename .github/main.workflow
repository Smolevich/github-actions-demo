...
action "aws test" {
  ...
  args = "lambda create-function --region $AWS_DEFAULT_REGION --function-name lambda-handler --memory 128 --role arn:aws:iam::$AWS_ACCOUNT_ID:user/$AWS_EXECUTION_ROLE --runtime go1.x --zip-file handler.zip --handler lambda-handler"
}